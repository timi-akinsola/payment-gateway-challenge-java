...
public class PaymentGatewayIntegrationTest {
  ...
  static GenericContainer<?> bankSimulator =
    ...
    .withCopyFileToContainer(MountableFile.forHostPath("imposters/bank_simulator.ejs"), "/imposters/bank_simulator.ejs")
  ...
  static void bankSimulatorProperties(DynamicPropertyRegistry registry) {
    registry.add("bank.simulator.url", 
      () -> String.format("http://%s:%d/payments", bankSimulator.getHost(), bankSimulator.getMappedPort(8080)));
  }
  ...
  void paymentCanBeRetrievedAfterBeingAuthorized() throws Exception {
    String responseJson = mvc.perform(MockMvcRequestBuilders.post("/payments")
      .contentType(MediaType.APPLICATION_JSON)
      .content(validPaymentRequestJson("01234567898765", "GBP")))
    .andExpect(status().isCreated())
    .andReturn()
